...
void Html_tag_open_table(DilloHtml *html, const char *tag, int tagsize)
{
   ...
   if (border != -1) {
      ...
      cpp_styleEngineSetNonCssHintOfNodeLength(html->styleEngine, CSS_PROPERTY_BORDER_TOP_WIDTH,    cssLength);
      cpp_styleEngineSetNonCssHintOfNodeLength(html->styleEngine, CSS_PROPERTY_BORDER_BOTTOM_WIDTH, cssLength);
      cpp_styleEngineSetNonCssHintOfNodeLength(html->styleEngine, CSS_PROPERTY_BORDER_LEFT_WIDTH,   cssLength);
      cpp_styleEngineSetNonCssHintOfNodeLength(html->styleEngine, CSS_PROPERTY_BORDER_RIGHT_WIDTH,  cssLength);
      cpp_styleEngineSetNonCssHintOfNodeEnum(html->styleEngine, CSS_PROPERTY_BORDER_TOP_STYLE,    BORDER_OUTSET);
      cpp_styleEngineSetNonCssHintOfNodeEnum(html->styleEngine, CSS_PROPERTY_BORDER_BOTTOM_STYLE, BORDER_OUTSET);
      cpp_styleEngineSetNonCssHintOfNodeEnum(html->styleEngine, CSS_PROPERTY_BORDER_LEFT_STYLE,   BORDER_OUTSET);
      cpp_styleEngineSetNonCssHintOfNodeEnum(html->styleEngine, CSS_PROPERTY_BORDER_RIGHT_STYLE,  BORDER_OUTSET);
   }

   if (cellspacing != -1) {
      cssLength = cpp_cssCreateLength(cellspacing, CSS_LENGTH_TYPE_PX);
      cpp_styleEngineSetNonCssHintOfNodeLength(html->styleEngine, CSS_PROPERTY_BORDER_SPACING, cssLength);
   }

   if ((attr_value = html_attribute_get_value(tag, tagsize, "width"))) {
      CssLength width = html_parse_attribute_width_or_height(attr_value);
      cpp_styleEngineSetNonCssHintOfNodeLength(html->styleEngine, CSS_PROPERTY_WIDTH, width);
      if (html->doctype.c_doc_type == DT_HTML && html->doctype.c_doc_type_version >= 5.0f)
   ...
   if ((attr_value = html_attribute_get_value(tag, tagsize, "align"))) {
      if (dStrAsciiCasecmp (attr_value, "left") == 0) {
         cpp_styleEngineSetNonCssHintOfNodeEnum(html->styleEngine, CSS_PROPERTY_TEXT_ALIGN, TEXT_ALIGN_LEFT);
      } else if (dStrAsciiCasecmp (attr_value, "right") == 0) {
         cpp_styleEngineSetNonCssHintOfNodeEnum(html->styleEngine, CSS_PROPERTY_TEXT_ALIGN, TEXT_ALIGN_RIGHT);
      } else if (dStrAsciiCasecmp (attr_value, "center") == 0) {
         cpp_styleEngineSetNonCssHintOfNodeEnum(html->styleEngine, CSS_PROPERTY_TEXT_ALIGN, TEXT_ALIGN_CENTER);
      }
      if (html->doctype.c_doc_type == DT_HTML && html->doctype.c_doc_type_version >= 5.0f) {
   ...
   if ((attr_value = html_attribute_get_value(tag, tagsize, "bgcolor"))) {
      ...
      if (bgcolor != -1) {
         cpp_styleEngineSetNonCssHintOfNodeColor(html->styleEngine, CSS_PROPERTY_BACKGROUND_COLOR, bgcolor);
      }
      if (html->doctype.c_doc_type == DT_HTML && html->doctype.c_doc_type_version >= 5.0f)
   ...
   if (border > 0) {
      ...
      cpp_styleEngineSetNonCssHintOfNodeLength(html->styleEngine, CSS_PROPERTY_BORDER_LEFT_WIDTH,   cssLength);
      cpp_styleEngineSetNonCssHintOfNodeLength(html->styleEngine, CSS_PROPERTY_BORDER_RIGHT_WIDTH,  cssLength);
      cpp_styleEngineSetNonCssHintOfNodeEnum(html->styleEngine, CSS_PROPERTY_BORDER_TOP_STYLE,    BORDER_INSET);
      cpp_styleEngineSetNonCssHintOfNodeEnum(html->styleEngine, CSS_PROPERTY_BORDER_BOTTOM_STYLE, BORDER_INSET);
      cpp_styleEngineSetNonCssHintOfNodeEnum(html->styleEngine, CSS_PROPERTY_BORDER_LEFT_STYLE,   BORDER_INSET);
      cpp_styleEngineSetNonCssHintOfNodeEnum(html->styleEngine, CSS_PROPERTY_BORDER_RIGHT_STYLE,  BORDER_INSET);
   }

   if (cellpadding != -1) {
      cssLength = cpp_cssCreateLength(cellpadding, CSS_LENGTH_TYPE_PX);
      cpp_styleEngineSetNonCssHintOfNodeLength(html->styleEngine, CSS_PROPERTY_PADDING_TOP,    cssLength);
      cpp_styleEngineSetNonCssHintOfNodeLength(html->styleEngine, CSS_PROPERTY_PADDING_BOTTOM, cssLength);
      cpp_styleEngineSetNonCssHintOfNodeLength(html->styleEngine, CSS_PROPERTY_PADDING_LEFT,   cssLength);
      cpp_styleEngineSetNonCssHintOfNodeLength(html->styleEngine, CSS_PROPERTY_PADDING_RIGHT,  cssLength);
   }
...
void Html_tag_open_tr(DilloHtml *html, const char *tag, int tagsize)
{
   ...
   case DILLO_HTML_TABLE_MODE_TD:
      ...
      if (html_attribute_get_value(tag, tagsize, "align")) {
         ...
         a_Html_tag_set_align_attr(&html->doctype, html->styleEngine, tag, tagsize);
      }
...
static void Html_set_collapsing_border_model(DilloHtml *html, Widget *col_tb)
{
   ...
   ffiStyleAttrsBorderWidth(html->styleEngine->getStyle (html->bw)->c_style_attrs_ref, &borderWidth);

   c_style_margin_t tableStyleMargin = {};
   ffiStyleAttrsMargin(tableStyle->c_style_attrs_ref, &tableStyleMargin);
   int marginWidth = tableStyleMargin.top;


   dw::core::style::StyleAttrs collapseCellAttrs = *(html->styleEngine->getStyle (html->bw));
   ffiStyleAttrsSetMargin(collapseCellAttrs.c_style_attrs_ref, 0);
   ffiStyleAttrsSetCollapseCellAttrs(collapseCellAttrs.c_style_attrs_ref, borderWidth.top);
   ...
   if (Html_table_get_border_model(html) != DILLO_HTML_TABLE_BORDER_COLLAPSE) {
      ...
      ffiStyleAttrsSetMargin(collapseTableAttrs.c_style_attrs_ref, marginWidth);
      collapseTableAttrs.borderColor = collapseCellAttrs.borderColor;

      ffiStyleAttrsSetCollapseTableAttrs(collapseTableAttrs.c_style_attrs_ref, collapseCellAttrs.c_style_attrs_ref, borderWidth.top);
...
static void Html_set_separate_border_model(DilloHtml *html, Widget *col_tb)
{
   ...
   ffiStyleAttrsSetMargin(separateCellAttrs.c_style_attrs_ref, 0);
                                     ...
                                     dw::core::style::TextAlignType text_align)
{
   ...
   case DILLO_HTML_TABLE_MODE_TD:
      ...
      if (!TopOfParsingStack(html)->table_context.cell_text_align_set) {
         cpp_styleEngineSetNonCssHintOfNodeEnum(html->styleEngine, CSS_PROPERTY_TEXT_ALIGN, text_align);
      }
      if (html_attribute_get_value(tag, tagsize, "nowrap")) {
         ...
         cpp_styleEngineSetNonCssHintOfNodeEnum(html->styleEngine, CSS_PROPERTY_WHITE_SPACE, WHITE_SPACE_NOWRAP);
      }
                                     ...
                                     const char *tag, int tagsize)
{
   ...
   case DILLO_HTML_TABLE_MODE_TD:
      ...
      if (ffiStyleAttrsTextAlign(html->styleEngine->getStyle (html->bw)->c_style_attrs_ref) == TEXT_ALIGN_STRING)
      ...
      if (ffiStyleAttrsBorderCollapse(html->styleEngine->getStyle(html->bw)->c_style_attrs_ref) == BORDER_MODEL_COLLAPSE){
